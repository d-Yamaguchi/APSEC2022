...
void postPlugin(final boolean isPing) throws java.io.IOException {
    ...
    final boolean onlineMode = _CVAR0.getOnlineMode();// TRUE if online mode is enabled
    ...
    if (graphs.size() > 0) {
        synchronized(graphs) {
            ...
            while (iter.hasNext()) {
                final ch.njol.skript.Metrics.Graph graph = iter.next();
                final java.lang.StringBuilder graphJson = new java.lang.StringBuilder();
                graphJson.append('{');
                for (final ch.njol.skript.Metrics.Plotter plotter : graph.getPlotters()) {
                    ch.njol.skript.Metrics.appendJSONPair(graphJson, plotter.getColumnName(), java.lang.Integer.toString(plotter.getValue()));
                }
                graphJson.append('}');
                if (!firstGraph) {
                    json.append(',');
                }
                json.append(ch.njol.skript.Metrics.escapeJSON(graph.getName()));
                json.append(':');
